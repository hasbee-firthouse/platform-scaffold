...
compatibility: Requires git, Node.js, pnpm, Docker, PostgreSQL, and Playwright.
metadata:
  author: platform-scaffold
  version: "1.0"
---
...
Use function calling / structured output (`response_format: { type: "json_schema", json_schema: ... }` for every LLM call. Never parse free-text responses with regex or string splitting.
...
- **Free-text LLM parsing** — Never use regex to parse LLM output. Use structured output (function calling / JSON mode).
